    ...
    (set, get) => ({
      ...
      setActiveChild: async (childId) => {
        ...
        try {
          ...
          const { error } = await setActiveChild(childId, parent.id);
          
          if (error) {
            console.error('Error setting active child in database:', error);
            return;
          }
          ...
          // Update other children's active status
          const updatedChildProfiles = get().childProfiles.map(c => ({
            ...c,
            is_active: c.id === childId
          }));
