...
// Function to delete AI generated content and its items
export async function deleteAIGeneratedContent(contentId: string): Promise<{
  success: boolean;
  error: Error | null;
}> {
  try {
    console.log(`[Database] Attempting to delete content with ID: ${contentId}`);

    // 1. Delete associated items from ai_content_items
    const { error: itemsError } = await supabase
      .from('ai_content_items')
      .delete()
      .eq('content_id', contentId);
    ...
    if (itemsError) {
      console.error(`[Database] Error deleting items for content ID ${contentId}:`, itemsError);
      throw itemsError; 
    }
    console.log(`[Database] Successfully deleted items for content ID: ${contentId}`);

    // 2. Delete the main content from ai_generated_contents
    ...
    if (contentError) {
      console.error(`[Database] Error deleting content ID ${contentId}:`, contentError);
      throw contentError;
    }
    console.log(`[Database] Successfully deleted content with ID: ${contentId}`);

    return { success: true, error: null };

  } catch (error) {
    console.error(`[Database] Failed to delete content ID ${contentId}:`, error);
